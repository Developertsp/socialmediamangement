...
// dd($token, $facebookId);
    ...
    public function postFacebook(Request $request)
    {
        ...
        if ($images !== null) {
            ...
            // Check if facebook_carousel is true
            if ($facebookCarousel) {
                // Prepare the child attachments for the carousel
                $childAttachments = [];
                foreach ($imageData as $image) {
                    $childAttachments[] = [
                        'link' => asset('storage/' . $image['path']),
                        'picture' => asset('storage/' . $image['path'])
                    ];
                }
            }
                    ...
                    try {
                        $response = $client->post("https://graph.facebook.com/v19.0/{$page['id']}/feed", [
                            'form_params' => [
                                ...
                                'caption' => $message,
                                'access_token' => $page['access_token'],
                                // If carousel is enabled, include the child_attachments parameter
                                // 'child_attachments' => $facebookCarousel ? json_encode($childAttachments) : $imageUrl
